...
def migrate_fn(spec, status, name, old, new, logger, namespace, **_):
    ...
    if old == False and new == True:
        ...
        for config in next_deployment_configs:
            if config.get("kind") == "Deployment":
                ...
                status_patch["create_fn"]["child-deployment-namespace"] = next_deployment_namespace
                status_patch["create_fn"]["child-deployment-app-name"] = config.get("metadata").get("labels").get("app")
                status_patch["create_fn"]["child-deployment-prometheus-url"] = config.get("spec").get("template").get("metadata").get("annotations").get("prometheusUrl")
                status_patch["create_fn"]["child-deployment-affinity"] = next_deployment_affinity
                
                # set rsync env vars to trigger rsync in init container
                init_container = config.get("spec").get("template").get("spec").get("initContainers")[0]
                init_container["env"] = [
                    {
                        "name": "RSYNC_SOURCE",
                        "value": current_deployment_service_name
                    },
                    {
                        "name": "RSYNC_SOURCE_PATH",
                        "value": "dt_data/dump.json"
                    },
                    {
                        "name": "RSYNC_DEST_PATH",
                        "value": "/var/tmp/dt_data"
                    }
                ]

                config["spec"]["template"]["spec"]["initContainers"][0].update(init_container)
        ...
        # patch the status of cpa for future migrations
        # resp = k8s_client_custom_object.patch_namespaced_custom_object("test.dev", "v1", namespace, "cyberphysicalapplications", name, body=status_patch)
